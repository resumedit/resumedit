 ...
 * @param {NestedItemClientStateType[]} arr - Array of Items
 * @param {string} id - id of item object
 * @returns Index of item object in the items array
 */
export const findItemIndexByClientId = (
  arr: NestedItemStoreDescendantListType<NestedItemDescendantClientStateType, NestedItemDescendantClientStateType>,
  id: IdSchemaType,
): number => {
...
const getOrderParams = (items: NestedItemOrderableChildClientStateType[]) => {
...
export const getOrderValueForAppending = (items: NestedItemOrderableChildClientStateType[]): number => {
  if (items.length === 0) {
...
export const randomizeOrderValue = (order: number, { randomOffsetMax }: OrderParams): number => {
  const randomOffset = Math.random() * randomOffsetMax;
  const randomizedOrder = order + randomOffset;
  return randomizedOrder;
};

export const reBalanceListOrderValues = (
  items: NestedItemOrderableChildClientStateType[],
  force?: boolean,
): NestedItemOrderableChildClientStateType[] => {
  ...
  const itemsWithOrderValuesReset = items.map((item, index) => {
    ...
    const orderedItem = {
      order: newOrder,
      disposition:
        item.disposition === NestedItemDisposition.Synced ? NestedItemDisposition.Modified : item.disposition,
      ...otherProps,
    };
...
const getCenteredOrderValue = (prevOrder: number, nextOrder: number | undefined, orderParams: OrderParams) => {
...
const getLastOrderValue = (prevOrder: number, orderParams: OrderParams) => {
...
const getAdjustedOrderValue = (
  item: NestedItemOrderableChildClientStateType,
...
const adjustListOrderValues = (
  ...
  return items.map((item, index): NestedItemOrderableChildClientStateType => {
...
const validateListOrderValues = (items: NestedItemOrderableChildClientStateType[]): boolean => {
...
export const updateListOrderValues = (
  items: NestedItemOrderableChildClientStateType[],
): NestedItemOrderableChildClientStateType[] => {
