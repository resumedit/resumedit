...
import { getDescendantModel, itemDescendantModelHierarchy } from "@/types/itemDescendant";
import { ItemDescendantServerComponentProps } from "./ItemDescendant.server";
...
export default async function ItemDescendantScaffoldServerComponent({
  ...
  if (rootItemModel === itemDescendantModelHierarchy[0]) {
    ...
    if (itemModel && derivedItemId) {
      const serverState = await getItemDescendantList(itemModel, derivedItemId);
